...
namespace Common.Tools
{
    ...
    public class JsonHelper
    {
        ...
        /// <summary>
        /// 解析JSON字符串生成对象实体注意点：如果其中有数组，或对象，则该对象必须有默认的构造函数或者只能有一个有参数的构造函数，否则提示无法计算表达式的值
        /// </summary>
        /// <typeparam name="T">对象类型</typeparam>
        /// <param name="json">json字符串(eg.{"ID":"112","Name":"石子儿",result:[{"":""},{"":""}]}等)</param>
            ...
            using (JsonWriter jw = new JsonTextWriter(sw))
            {
                ...
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        {
                            jw.WritePropertyName(p.Name);
                            string values = p.GetValue(list[i], null).ToString();
