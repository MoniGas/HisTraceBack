...
namespace Common.Tools
{
    public class UploadImage
    {
        ...
        public static void CreateNewImage(double newWidth, double newHeight, Image initImage, string watermarkText, string watermarkImage, WaterMarkPosition waterTextPosition, WaterMarkPosition waterImagePosition, string savePath)
        {
            //生成新图 
            //新建一个bmp图片 
            ...
            if (File.Exists(HttpContext.Current.Server.MapPath("~" + watermarkImage)))
            {
                ...
                using (System.Drawing.Image wrImage = System.Drawing.Image.FromFile(HttpContext.Current.Server.MapPath("~" + watermarkImage)))
                {
                    ...
                    if (initImage.Width >= wrImage.Width && initImage.Height >= wrImage.Height)
                    {
                        ...
                        gWater.DrawImage(wrImage, new Rectangle(wrp_X, wrp_Y, wrImage.Width, wrImage.Height), 0, 0, wrImage.Width, wrImage.Height, GraphicsUnit.Pixel, imgAttributes);
        ...
        public static void MakeThumbnail(string originalImagePath, string thumbnailPath, int width, int height, string mode)
        {
            ...
            #region 20140515
            Image originalImage;
            if (originalImagePath.LastIndexOf("file.i-oid.org.cn") < 0)
            {
                originalImage = Image.FromFile(System.Web.HttpContext.Current.Server.MapPath(originalImagePath));
            }
            else
            {
                System.Net.WebClient wc = new System.Net.WebClient();
                byte[] myData = wc.DownloadData(originalImagePath);
                System.IO.MemoryStream myStream = new System.IO.MemoryStream(myData);
                originalImage = Image.FromStream(myStream);
                myStream.Close();
            }
            #endregion
            ...
            try
            {
                ...
                string url = System.Web.HttpContext.Current.Server.MapPath(thumbnailPath.Substring(0, originalImagePath.LastIndexOf("/")));
        ...
        public static Image GetAutoControlWideImg(Image initImage, System.Double targetWidth, string watermarkText, string watermarkImage, WaterMarkPosition waterTextPosition, WaterMarkPosition waterImagePosition)
        {
            //原图宽高均小于模版，不作处理，直接保存 
            if (initImage.Width <= targetWidth)
            {
            ...
            else
            {
                ...
                return CreateNewImage(newWidth, newHeight, initImage, watermarkText, watermarkImage, waterTextPosition, waterImagePosition);
            }
        }


        public static Image CreateNewImage(double newWidth, double newHeight, Image initImage, string watermarkText, string watermarkImage, WaterMarkPosition waterTextPosition, WaterMarkPosition waterImagePosition)
        {
            //生成新图 
            //新建一个bmp图片 
            System.Drawing.Image newImage = new System.Drawing.Bitmap((int)newWidth, (int)newHeight);
