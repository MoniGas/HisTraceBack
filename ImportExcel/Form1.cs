...
namespace ImportExcel
{
    ...
    {
        static InterfaceWeb.WebClient _WebClient = null;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string con = System.Configuration.ConfigurationManager.AppSettings["WebConnect"].ToString();
            string scanDirectoryPath = this.textBox1.Text;
            BLL.UDIMaterialBLL uma = new BLL.UDIMaterialBLL();
            //指定的文件夹目录
            ...
            else
            {
                ...
                {
                    result = uma.SaveUDIMaterial(fsinfo.FullName);
                    if (result.code == "0")
                    {
                        this.textBox2.AppendText(fsinfo.Name + "于" + DateTime.Now + "-" + result.Msg);
                        this.textBox2.AppendText("\r\n");
                    }

                }
                this.textBox2.AppendText("执行完毕！");
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            th.Start();
            if (this.textBox2.Text.Contains("全部执行完毕"))
            {
                th.Abort();
            }
        ...
        public static void DOGetUDIData()
        {
            ...
            //连接库
             ...
             WriteLog("同步国药监数据服务启动", "GetUDIData");
             frm.textBox2.AppendText("同步国药监数据服务启动");
             frm.textBox2.AppendText("\r\n");
            try
            {
                using (SqlConnection conn = new SqlConnection(conStr))
                {
                    ...
                    while (PageCount <= Convert.ToInt32(GetAllUDIData.totalPageCount))
                    {
                        ...
                        if (Convert.ToInt32(currentTime) < Convert.ToInt32(DateTime.Now.ToString("yyyyMMdd")))
                        {
                            ...
                            {
                                WriteLog(resToken.returnMsg, "GetUDIData");
                            ...
                            else
                            {
                                ...
                                WriteLog(resToken.currentTime + "日获取token成功！", "GetUDIData");
                            }
                            ...
                            if (GetAllUDIData.dataSet != null)
                            {
                                ...
                                if (GetAllUDIData.dataSet.deviceInfo.Count > 0)
                                {
                                    foreach (GetAllUDIData item in GetAllUDIData.dataSet.deviceInfo)
                                    {
                                        ...
                                        {
                                            WriteLog(ex.Message + "异常sql:" + sql, "GetUDIData");
                                            frm.textBox2.AppendText(ex.Message + "异常sql:" + sql);
                                            frm.textBox2.AppendText("\r\n");
                                        }
                                    }
                                }
                                WriteLog("第" + PageCount + "页同步成功，共同步" + num + "条", "GetUDIData");
                                num = 0;
                            }
                        }
                        PageCount++;
                ...
                PageCount = 1;
                WriteLog("执行完成，共计" + GetAllUDIData.totalRecordCount + "条数据!", "GetUDIData");
                DoUpdateDIData();
            }
            catch (Exception)
            {
        ...
        public static void WriteLog(string msg, string directoryName)
        {
            ...
            using (StreamWriter sw = new StreamWriter(errlogpath + "\\" + DateTime.Now.ToString("dd") + "error.txt", true))
            {
        ...
        public static TokenResult GetToken(string appId, string appSecret, string TYSHXYDM)
        {
            ...
            if (!string.IsNullOrEmpty(appId) && !string.IsNullOrEmpty(appSecret) && !string.IsNullOrEmpty(TYSHXYDM))
            {
                ...
                if (_WebClient == null)
                {
                    _WebClient = new InterfaceWeb.WebClient { Encoding = Encoding.UTF8 };
                }
                try
                {
                    ...
                    string strResult = InterfaceWeb.WebClient.sendPost(UDI_URL + functionUrl, param, "post");
        ...
        {
            Form1 frm = new Form1();
            WriteLog("同步国药监数据服务启动-更新" + sql, "UpdateDIData");
            //一旦启动，将会一直执行
            while (true)
            {
                try
                {
                    using (SqlConnection conn = new SqlConnection(conStr))
                    {
                        ...
                        while (PageCount <= Convert.ToInt32(GetAllUDIData.totalPageCount))
                        {
                            ...
                            if (Convert.ToInt32(currentTime) < Convert.ToInt32(DateTime.Now.ToString("yyyyMMdd")))
                            {
                                ...
                                if (resToken.returnCode != 1 && resToken.returnCode != 9)
                                {
                                    WriteLog(resToken.returnMsg, "UpdateDIData");
                                }
                                else
                                {
                                    accessToken = resToken.accessToken;
                                    currentTime = resToken.currentTime;
                                    WriteLog(resToken.currentTime + "日获取token成功！", "UpdateDIData");
                                }
                            ...
                            if (GetAllUDIData.returnCode == 1)
                            {
                                if (GetAllUDIData.dataSet.deviceInfo.Count > 0)
                                {
                                    foreach (GetAllUDIData item in GetAllUDIData.dataSet.deviceInfo)
                                    {
                                        ...
                                        {
                                            WriteLog(ex.Message + "异常sql:" + sql, "UpdateDIData");
                                        }
                                    }
                                }
                                WriteLog("第" + PageCount + "页同步成功，共同步" + num + "条", "UpdateDIData");
                                num = 0;
                            }
                            PageCount++;
                    ...
                    PageCount = 1;
                    WriteLog("执行完成，共计" + GetAllUDIData.totalRecordCount + "条数据!", "UpdateDIData");
                    //每隔一定时间执行一次(1000 * timeSpan * 60 * 60 为一小时)
                }
                catch (Exception ex)
                {
                    WriteLog(ex.Message, "UpdateDIData");
                }
            }
        }
        #endregion
        ...
        public static GetAllUDIInfo GetAllUDIInfo(string access_token, int currentPageNumber, int type)
        {
            ...
            if (!string.IsNullOrEmpty(access_token))
            {
                ...
                try
                {
                    ...
                    {
                        WriteLog(info.returnMsg, "GetAllUDIInfo");
                        frm.textBox2.AppendText(info.returnMsg);
                        frm.textBox2.AppendText("\r\n");
