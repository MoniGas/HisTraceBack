define(['plugins/router', 'knockout', 'knockout.mapping', 'jquery', 'jqPaginator', 'bootstrap-datepicker', './material_order_del', './material_order_edit', "./material_order_info", 'plugins/dialog', 'utils', 'jquery.fileDownload', 'logininfo'],
    function (router, ko, km, $, jq, bdp, material_order_del, material_order_edit, material_order_info, dialog, utils, jfd, loginInfo) {
        var moduleInfo = {
            moduleID: '20000',
        ...
        var getData = function (name, status, bDate, eDate, pageIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "/Admin_Material_OnlineOrder/Index",
        ...
        var editOrder = function (id, data, event) {
            ...
            material_order_edit.show(id).then(function () {
                var list = getDataKO(name(), bDate(), eDate(), vm.vmOrder.pageIndex());
                updateData(list);
            });
        };
        var delOrder = function (id, data, event) {
            var currentObj = $(event.target);
            currentObj.blur();
            var id = ko.utils.unwrapObservable(id);
            dialog.showMessage("确定隐藏该订单吗？", '系统提示', [
            {
                title: '确定',
                callback: function () {
                    var sendData = {
        ...
        var infoOrder = function (id, data, event) {
            //alert(1);
        };
        ...
        var mouseoverFun = function (data, event) {
            ...
            var ShowSend = self.find("div[eleflag='ShowSend']");
            var ShowDel = self.find("div[eleflag='ShowDel']");

            if (data.OrderType() == 1 || data.OrderType() == 3 || data.OrderType() == 4 || data.OrderType() == 8) {
                ShowSend.css({ "display": "inline" });
            }
            if (data.OrderType() == 0) {
                ShowDel.css({ "display": "inline" });
            }
        }
        ...
        var exportExcel = function () {
            ...
            $.fileDownload('/Admin_Material_OnlineOrder/ExportExcel?name=' + $("#searchName").val() + '&status=' + status + '&bDate=' + $("#bDate").val() + '&eDate=' + $("#eDate").val())
