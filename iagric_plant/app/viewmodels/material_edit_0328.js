define(['plugins/router', 'knockout', 'jquery', 'knockout.validation', 'utils', 'logininfo', 'plugins/dialog', 'jquery-ui', 'jquery.uploadify', 'kindeditor.zh-CN'],
    function (router, ko, $, kv, utils, loginInfo, dialog, jqueryui, uploadify, kcn) {
        ...
        var vmMaterial = function (id) {
            ...
            self.init = function () {
                ...
                $.ajax({
                    ...
                    success: function (jsonResult) {
                        ...
                        self.selectedBrand(jsonResult.ObjModel.Brand_ID);
                        self.selectedprocess(jsonResult.ObjModel.ProcessID);
                        self.materialSpec(jsonResult.ObjModel.MaterialSpecification);
                        self.materialPrice(jsonResult.ObjModel.price);
                        var shelf = jsonResult.ObjModel.ShelfLife;
                        var shelfValue = "";
                        var shelfUnit = "长期";
                        //alert(shelf.indexOf(shelfUnit));
                        ...
                        self.selectedShelf = ko.observable(shelfUnit);
                        self.materialShelfLife = ko.observable(shelfValue);

                        self.materialMemo(jsonResult.ObjModel.Memo);
            ...
            self.SaveMaterial = function (data, event) {
                ...
                if (self.errors().length <= 0) {
                    ...
                    if (!((!(b == "长期" || b == "视存储环境") && (a == "" || a == null)) || (self.files() == ""))) {
        ...
        vmMaterial.prototype.binding = function () {
        ...
        vmMaterial.prototype.delImage = function (data, event) {
        ...
        vmMaterial.prototype.AddProperty = function (data, event) {
        ...
        vmMaterial.prototype.delProperty = function (data, event) {
            var index = vmObj.property.indexOf(data);
            vmObj.property.splice(index, 1);
        }
        vmMaterial.prototype.close = function () {
            $("#image_upload").uploadify('destroy');
            dialog.close(this);
        }
        vmMaterial.show = function (id) {
            vmObj = new vmMaterial(id);
