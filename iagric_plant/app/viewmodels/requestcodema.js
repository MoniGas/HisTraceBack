define(['plugins/router', 'durandal/system', 'knockout', 'knockout.mapping', 'jquery', 'jqPaginator', 'utils', 'bootstrap-datepicker', 'jquery.poshytip', 'logininfo', 'plugins/dialog'],
function (router, system, ko, km, $, jq, utils, bdp, poshytip, loginInfo, dialog) {
    var moduleInfo = {
        moduleID: '98000',
    ...
    //跳转子码段页面
    var goTozicodeseting = function (requestcodeID, data, event) {
        var currentObj = $(event.target);
        currentObj.blur();
        requestcodeID = ko.utils.unwrapObservable(requestcodeID);
        router.navigate('#zicodeseting?requestcodeid=' + requestcodeID);
    }
    //跳转拆分页面
    var goTorequestcodesetting = function (requestcodeID, data, event) {
        var currentObj = $(event.target);
        currentObj.blur();
        requestcodeID = ko.utils.unwrapObservable(requestcodeID);
        router.navigate('#requestcodesetting?requestcodeid=' + requestcodeID);
    }
    //自定义绑定-分页控件
    ko.bindingHandlers.requestcodemaPager = {
    ...
    var mouseoverFun = function (data, event) {
        // 删除
        var btnDel = $("#btnDel_" + data.RequestCode_ID());
        // 修改
        var btnEdit = $("#btnEdit_" + data.RequestCode_ID());
    ...
    var vm = {
        ...
        goTozicodeseting: goTozicodeseting,
