...
(function ($, undefined) {
    var sidebar_count = 0;
    var click_event = touch && $.fn.tap ? 'tap' : 'click';
    ...
    var helper = {
        ...
        getAttrSettings: function (el, attr_list, prefix) {
            ...
            for (var li in attr_list) if (attr_list.hasOwnProperty(li)) {
                ...
                var attr_val, attr_name = helper.unCamelCase(name.replace(/[^A-Za-z0-9]{1,}/g, '-')).toLowerCase()

                if (!((attr_val = el.getAttribute(prefix + attr_name)))) continue;
                settings[name] = helper.strToVal(attr_val);
            }
            return settings;
        },
        scrollTop: function () {
    ...
    function Sidebar(sidebar, settings) {
        ...
        var attrib_values = helper.getAttrSettings(sidebar, $.fn.ace_sidebar.defaults, 'sidebar-');
        ...
        var toggleIcon = function (minimized) {
            var icon = $(this).find('.ace-icon'), icon1, icon2;
            if (icon.length > 0) {
        ...
        this.toggleMenu = function (toggle_btn, save) {
            ...
            if (document.all && !document.addEventListener) helper.redraw(sidebar);
        }
        ...
        this.toggleResponsive = function (toggle_btn) {
            ...
            if (toggle_btn) {
                var icon = $(toggle_btn).find('.ace-icon'), icon1, icon2;
                if (icon.length > 0) {
                    icon1 = icon.attr('data-icon1'); //the icon for expanded state
                    icon2 = icon.attr('data-icon2'); //the icon for collapsed state
        ...
        //toggling submenu
        this.$sidebar.on(click_event + '.ace.submenu', '.nav-list', function (ev) {
            var nav_list = this;
            ...
            if (!link_element.hasClass('dropdown-toggle')) {//it doesn't have a submenu return
                ...
                var ios_safari = !!agent.match(/OS ([4-9])(_\d)+ like Mac OS X/i) && !agent.match(/CriOS/i)
                if (ios_safari && link_element.attr('data-link') !== 'false') {
        ...
        this.show = function (sub, $duration, shouldWait) {
            ...
            var safari = !!agent.match(/Safari/i) && !agent.match(/Chrome/i);
            var android = safari && !!agent.match(/Android/i)
            if (android) {
                setTimeout(function () {
                    complete(null, false);
                    helper.redraw(sub);
    ...
    $(document)
	.on(click_event + '.ace.menu', '.menu-toggler', function (e) {
	    var btn = $(this);
	    ...
	    if (btn.hasClass('display')) {
	        ...
	        if (auto_hide) $(document).off(click_event1);
	    }

	    return false;
	})
    //sidebar collapse/expand button
	.on(click_event + '.ace.menu', '.sidebar-collapse', function (e) {
	...
	.on(click_event + '.ace.menu', '.sidebar-expand', function (e) {
	    var target = $(this).attr('data-target'), $sidebar = null;
	    if (target) $sidebar = $(target);
	    if ($sidebar == null || $sidebar.length == 0) $sidebar = $(this).closest('.sidebar');
	    if ($sidebar.length == 0) return;
