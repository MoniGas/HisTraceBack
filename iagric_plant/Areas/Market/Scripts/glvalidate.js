...
function checkMobile(s)// 正则判断
{
    ...
    var pattern = /^(^[0-9]{3,4}[0-9]{3,8}$)|(^[0-9]{3,4}\s[0-9]{3,8}$)|(^[0-9]{3,4}\-[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)|1[3|4|5|7|8]\d{9}$/;
    if (pattern.test(s))
    { return true; }
    else
    { return false; }
...
function checkContactNumber(mobile) {
    var isPhoneT = /^(^[0-9]{3,4}[0-9]{3,8}$)|(^[0-9]{3,4}\s[0-9]{3,8}$)|(^[0-9]{3,4}\-[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)|1[3|4|5|7|8]\d{9}$/;
    if (!isPhoneT.test(mobile)) {
        return false;
    }
    else {
        return true;
...
如果验证成功返回true,否则返回false 
*/
function isWetChat(s) {
    var regu = "^[a-zA-Z0-9_]{5,20}$";
    var re = new RegExp(regu);
    if (re.test(s)) {
        return true;
    } else {
