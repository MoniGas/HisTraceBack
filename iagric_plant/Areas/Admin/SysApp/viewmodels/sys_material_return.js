define(['plugins/router', 'knockout', 'knockout.mapping', 'jquery', 'jqPaginator', 'bootstrap-datepicker', 'plugins/dialog', 'utils', 'jquery.fileDownload', 'logininfo'],
    function (router, ko, km, $, jq, bdp, dialog, utils, jfd, loginInfo) {
        var moduleInfo = {
            moduleID: '22000',
        ...
        var rePayOrder = function (orderNum, monry, memo) {
            //alert(1);
            //Detail_data = "2016011521001004450065234744^0.01^协商退款";
            //Batch_num = "1";
            //Batch_no = "201601190000001";

            var detail = ko.utils.unwrapObservable(orderNum) + '^' + ko.utils.unwrapObservable(monry) + '^' + ko.utils.unwrapObservable(memo)
            //alert(detail);
            var date = new Date();
            ...
            var currentdate = year.toString() + month.toString() + strDate.toString() + date.getHours().toString() + date.getMinutes().toString()
             + date.getSeconds().toString() + date.getMilliseconds().toString();

            window.open("/Refund/Refund?Detail_data=" + detail + "&Batch_num=1&Batch_no=" + currentdate);

            dialog.showMessage("您确定已经退款成功了吗？", '系统提示', [
                {
                    title: '确定',
                    callback: function () {
                        var currentPageRow = vm.vmOrder.ObjList().length;
        ...
        statusArray.selectstatus.subscribe(function () {
            //alert(statusArray.selectstatus().statusId);
        });
        var vm = {
            binding: function () {
                //初初化导航状态
