...
//             }
         ...
         self.init = function () {
             ...
             $.ajax({
                 type: "POST",
                 url: "/SysSupervision/GetModelInfo",
                 contentType: "application/json;charset=utf-8", //必须有
                 dataType: "json", //表示返回值类型，不必须
                 data: JSON.stringify(sendData),
                 async: false,
                 success: function (jsonResult) {
                     ...
                     //                         return;
                     //                     };
                     self.companyName(jsonResult.ObjModel.CompanyName);
                     self.centerAddress(jsonResult.ObjModel.CenterAddress);
                     self.linkMan(jsonResult.ObjModel.LinkMan);
                     self.linkPhone(jsonResult.ObjModel.LinkPhone);
                     self.complaintPhone(jsonResult.ObjModel.ComplaintPhone);
                     self.postalCode(jsonResult.ObjModel.PostalCode);
     ...
     vmUser.show = function (eId) {
         var vmObj = new vmUser(eId);
         vmObj.init();
