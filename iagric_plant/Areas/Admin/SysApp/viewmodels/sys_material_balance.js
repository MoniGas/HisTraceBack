define(['plugins/router', 'knockout', 'jquery', 'bootstrap-datepicker', 'jquery.uploadify', 'knockout.validation', 'utils', 'logininfo', 'knockout.mapping', 'jqPaginator', 'plugins/dialog', './sys_material_balance_info'],
    function (router, ko, $, bdp, uploadify, kv, utils, loginInfo, km, jq, dialog, sellcode_info) {
        ...
        var payOrder = function (id, getAccount, totalMoney, getTrueName, data, event) {
            var currentObj = $(event.target);
            currentObj.blur();
            var id = ko.utils.unwrapObservable(id);
            var getAccount = ko.utils.unwrapObservable(getAccount);
            var getTrueName = ko.utils.unwrapObservable(getTrueName);
            var totalMoney = ko.utils.unwrapObservable(totalMoney);
            dialog.showMessage("确定要结算该订单吗？", '系统提示', [
            {
                title: '确定',
                callback: function () {
            ...
            //            dialog.showMessage("确定已经结算过该订单吗？", '系统提示', [
            //            {
            //                title: '确定',
            //                callback: function () {
            //                    var sendData = {
            ...
            //                    $.ajax({
            //                        type: "POST",
            //                        url: "/Admin_Material_OnlineOrder/BalanceOrder",
            //                        contentType: "application/json;charset=utf-8", //必须有
            //                        dataType: "json", //表示返回值类型，不必须
            //                        data: JSON.stringify(sendData),
            //                        async: false,
            ...
            //                                dialog.showMessage(jsonResult.Msg, '系统提示', [{ title: '确定', callback: function () {
            //                                    if (jsonResult.code != 0) {
            //                                        var currentPageRow = vm.vmSellCode.ObjList().length;
            //                                        var pageIndex = vm.vmSellCode.pageIndex();
            //                                        if (currentPageRow - 1 == 0 && pageIndex > 1) {
            //                                            pageIndex = vm.vmSellCode.pageIndex() - 1;
            //                                        }
            //                                        var list = getDataKO(vm.vmSellCode.pageIndex());
            //                                        updateData(list);
            //                                        this.pageSize = parseInt(vm.vmSellCode.pageSize());
            //                                        this.totalCounts = parseInt(vm.vmSellCode.totalCounts());
            //                                    }
        ...
        var mouseoverFun = function (data, event) {
            ...
            var ShowPay = self.find("div[eleflag='ShowPay']");
