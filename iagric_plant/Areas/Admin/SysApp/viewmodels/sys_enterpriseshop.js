define(['plugins/dialog', 'plugins/router', 'knockout', 'knockout.mapping', 'jqPaginator', 'utils', 'logininfo', "./sys_addenterprise"],
function (dialog, router, ko, km, jq, utils, loginInfo, sys_addenterprise) {
    var moduleInfo = {
        moduleID: '11110',
    ...
    var getData = function (pageIndex, name) {
        ...
        $.ajax({
            type: "POST",
            url: "/SysEnterpriseManage/GetEnterpriseShop",
    ...
    var VerifyShop = function (enterpriseid, type, strType, data, event) {
        ...
        var msg = '您确定要' + strType + '该企业的商城服务吗？';
        dialog.showMessage(msg, '系统提示', [
            {
                ...
                callback: function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/SysEnterpriseManage/VerifyShop",
                        contentType: "application/json;charset=utf-8", //必须有
                        dataType: "json", //表示返回值类型，不必须
                        data: JSON.stringify(sendData),
                        async: false,
