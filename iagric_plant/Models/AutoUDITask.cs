...
namespace iagric_plant.Models
{
    public class AutoUDITask
    {
        static WebClient _WebClient = null;
        static string UDI_URL = System.Configuration.ConfigurationManager.AppSettings["UDI_URLNew"];
        ...
        {
            ////40秒执⾏⼀次
            //var timer = new System.Threading.Timer(DOGetUDIData, null, 0, 1000 * 40);
            DOGetUDIData();
        }
        #region 同步国药监数据服务-新增
        [MethodImpl(MethodImplOptions.Synchronized)]
        public void DOGetUDIData()
        {
            Log.WriteLog(DateTime.Now.ToShortTimeString() + "执行了", "GetUDIData");
            if (getTimeSpan(DateTime.Now.ToString()))
            {
                                            ...
                                            foreach (GetAllUDIData item in GetAllUDIData.dataSet.deviceInfo)
                                            {
                                                try
                                                {
                                                    using (SqlTransaction trans = conn.BeginTransaction())
                                                    {
                                                        ...
                                                        sql = @" select TOP 1 * from dbo.UDIMaterial where MinDI='" + item.zxxsdycpbs + "'";
            ...
            {
                Log.WriteLog("执行了，但没进去", "GetUDIData");
            }
        ...
        //                                        item.ylqxzcrbarmc = item.ylqxzcrbarmc.Contains("'") ? item.ylqxzcrbarmc.Replace("'", "''") : item.ylqxzcrbarmc;
        //                                        item.cpms = item.cpms.Contains("'") ? item.cpms.Replace("'", "''") : item.cpms;
        //                                        sql = @" select TOP 1 * from dbo.UDIMaterial where MinDI='" + item.zxxsdycpbs + "'";//根据最小销售单元产品标识查询本地库产品是否存在
        //                                        DataSet dtConn = new DataSet();
        //                                        SqlDataAdapter commandConn = new SqlDataAdapter(sql, conn);
        //                                        commandConn.SelectCommand.Transaction = trans;
        //                                        commandConn.Fill(dtConn);
        ...
        public static bool getTimeSpan(string timeStr)
        {
            ...
            string start = System.Configuration.ConfigurationManager.AppSettings["startTime"].ToString();
            string end = System.Configuration.ConfigurationManager.AppSettings["endTime"].ToString();
            TimeSpan dspStart = DateTime.Parse(start).TimeOfDay;
            TimeSpan dspEnd = DateTime.Parse(end).TimeOfDay;
