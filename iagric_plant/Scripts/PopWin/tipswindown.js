...
/*-----------------------------------------------------------------------
title:	  窗口标题
type:     显示的文件类型(以下几种){ text | id | img | url | iframe }
content:  内容
width:	  内容宽度
height:	  内容高度
...
/*------------------------------------------------------------------------
simpleWindown("例子","text:例子","500","400","true","3000","0")
------------------------------------------------------------------------*/

function tipsWindown(title, type, content, width, height, drag, time, showbg) {
    $("#windown-box").remove();         //请除内容

    var width = width >= 950 ? this.width = 950 : this.width = width;       //设置最大窗口宽度
    var height = height >= 500 ? this.height = 500 : this.height = height;  //设置最大窗口高度

    var simpleWindown_html = "";
    simpleWindown_html = "<div id='windownbg' style='height:" + $(document).height() + "px;filter:alpha(opacity=0);opacity:0;z-index: 10000'></div>";
    ...
    $("#windownbg").animate({ opacity: "0.4" }, 1); //设置透明度
