...
namespace iagric_plant.Controllers
{
    ...
    public class OlineAlipayController : Controller
    {
        ...
        public void Alipay(string out_trade_no, string subject, string total_fee, string show_url, string return_url)
        {
            ...
            string notify_url = System.Configuration.ConfigurationManager.AppSettings["IpRedirect"] + "OlineAlipay/Notify_url";
            //订单描述
            //选填
            ...
            WriteLog.WriteAlipayLog("支付购买产品信息：" + "订单编号out_trade_no【" + sParaTemp["out_trade_no"] + "】，购买产品subject【"+sParaTemp["subject"]+"】"
        ...
        public ActionResult Notify_url()
        {
            ...
            if (sPara.Count > 0)
            {
                ...
                if (verifyResult)
                {
                    ...
                    if (Request.Form["trade_status"] == "TRADE_FINISHED" || Request.Form["trade_status"] == "TRADE_SUCCESS")
                    {
                        ...
                        result = bll.PaySuccess(out_trade_no, (int)Common.EnumFile.PayStatus.Paid, trade_no, BuyerEmail, BuyerId);
                        msg = result.Msg;
                    }
                    else
                    {
                    ...
                    WriteLog.WriteAlipayLog("交易异步通知信息：商户订单号out_trade_no【" + out_trade_no +
            ...
            WriteLog.WriteAlipayLog("订单编号out_trade_no的交易结果：" + msg);
        ...
        public ActionResult Transfer(long checkId, string totalMoney, string getAccount, string getTrueName)
        {
            ...
            WriteLog.WriteAlipayLog("交易结果：" + sParaTemp);
            //Response.Write(sHtmlText);
        ...
        public ActionResult Transfer_url()
        {
            ...
            {
                msg = "出现异常错误！";
                WriteLog.WriteErrorLog("支付宝结算失败,出现异常错误！");
            }
            WriteLog.WriteErrorLog("交易结果：" + msg);
            WriteLog.WriteAlipayLog("交易结果：" + msg);
            rtr = RedirectToAction("Index", "Wap_Consumers");
            return rtr;
        }
        #endregion
        ...
        private SortedDictionary<string, string> GetRequestPost()
        {
            ...
            WriteLog.WriteAlipayLog(LogStr.ToString());
            return sArray;
        }
    }
}
